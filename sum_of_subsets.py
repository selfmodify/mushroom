...
def sum_of_subset_recursive(arr, total, i, matches):
    ...
    if total == 0:
        ...
        return 0    # We have reached the end of array, which means total is too big.
    elif total < 0:
        ...
        return (
            sum_of_subset_recursive(arr, total - arr[i], i-1, with_arr_i ) +    # Include the current element in the final solution
            sum_of_subset_recursive(arr, total, i-1, matches )               # Do not include the current element in the final solution.
        )
